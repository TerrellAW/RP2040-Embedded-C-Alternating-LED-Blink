...
// Define constants
const uint32_t GPIO0_MASK = 1;
const uint32_t GPIO25_MASK = 1 << 25;
...
void ctrl() {
	...
	SIO_BASE[36] = GPIO0_MASK;

	// Set SIO control over GPIO 25 (LED)
	CTRL_25 = 5;
	// Enable GPIO 25 (LED)
	SIO_BASE[36] = GPIO25_MASK;
...
void led_loop() {
	...
	while (1) {
		...
		// Turn on GPIO25
		SIO_BASE[20] = GPIO25_MASK;
		delay();

		// Turn off GPIO25
		SIO_BASE[24] = GPIO25_MASK;
		delay();

		// Turn on GPIO0
		SIO_BASE[20] = GPIO0_MASK;
		delay();

		// Turn off GPIO0
		SIO_BASE[24] = GPIO0_MASK;
		delay();
